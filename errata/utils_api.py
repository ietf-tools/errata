...
# Based on datatracker's ietf/api/ietf_utils.py
...
def requires_api_token(func_or_endpoint: Optional[Union[Callable, str]] = None):
    ...
    Usage:
        * Basic: endpoint defaults to the qualified name of the wrapped method. E.g., in errata.views,

                @requires_api_token
                def my_view(request):
                    ...

            will require a token for "errata.views.my_view"

        * Custom endpoint: specify the endpoint explicitly

                @requires_api_token("some_module.views.some_other_thing")
                def my_view(request):
                    ...

            will require a token for "some_module.views.some_other_thing"
    """
